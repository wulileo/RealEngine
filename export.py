...
with open(dir_path+"/"+object.name_full+'.mesh', 'wb') as f:
    #写入文件头
    f.write("mesh".encode())
    f.write(struct.pack('i',len(kVertexRemoveDumplicateVector)))
    f.write(struct.pack('i',len(kVertexIndexVector)))
    print(len(kVertexRemoveDumplicateVector),len(kVertexIndexVector))
    #写入顶点数据
